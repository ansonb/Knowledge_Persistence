...
class Trainer:
    ...
    def __init__(self, model, config):
        ...
        self.test_every = max(self.config.epochs//20, 1)
    ...
    def train_step_pairwise(self, pos_h, pos_r, pos_t, neg_h, neg_r, neg_t):
        ...
        if self.config.sampling == 'adversarial_negative_sampling':
            ...
            if self.config.neg_rate>1 and pos_preds.shape[0]<neg_preds.shape[0]:
                pos_preds = pos_preds.unsqueeze(-1).repeat([1,self.config.neg_rate])
                pos_preds = pos_preds.view(-1)
    ...
    def train_model(self):
        ...
        for cur_epoch_idx in range(self.config.epochs):
            ...
            if cur_epoch_idx%self.test_every==0:
                self.model.eval()
                self.evaluator.full_test(cur_epoch_idx)
                self.evaluator.metric_calculator.save_test_summary(self.model.model_name + '_{}'.format(cur_epoch_idx))   
                save_path = Path(os.path.join(self.config.path_embeddings, self.model.model_name + '_{}'.format(cur_epoch_idx)) )
                self.export_embeddings(save_path=save_path)
                self.save_model(epoch=cur_epoch_idx)
            ...
            if cur_epoch_idx % self.config.test_step == 0:
                ...
                if self.config.early_stopping and self.early_stopper.should_stop(metrics):
    ...
    def save_model(self, epoch=None):
        ...
        if epoch:
            saved_path = self.config.path_result / 'models' / self.model.model_name / str(epoch)
        else:
            saved_path = self.config.path_result / 'models' / self.model.model_name
    ...
    def export_embeddings(self, save_path=None):
        """
