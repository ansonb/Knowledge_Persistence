...
def get_sparse_graph_spoptim(preds, data, num_entities, num_relations, fraction=1.0):	
	if fraction<1.0:
		adj_tensor = {}
		thresh = -1e8
		indices = []
		...
		min_pred = np.min([preds[idx] for idx in indices])
...
def main():
	...
	neg_data = evaluator.sample_negative_rels(sampling='unif',neg_rate=1.0,pos_data_len=len(evaluator.test_data))

	preds_pos = evaluator.get_graph_with_edge_scores(data=evaluator.test_data, test_batch_size=args.batch_size)
	preds_neg = evaluator.get_graph_with_edge_scores(data=neg_data, test_batch_size=args.batch_size)

	pred_graph_pos, original_graph = get_sparse_graph_spoptim(preds_pos, evaluator.test_data, config.tot_entity, config.tot_relation, fraction=args.fraction)
	pred_graph_neg, _ = get_sparse_graph_spoptim(preds_neg, neg_data, config.tot_entity, config.tot_relation, fraction=args.fraction)
